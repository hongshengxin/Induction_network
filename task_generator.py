...
def train_omniglot_character_folders(trainfloder,labels):
    data_dict={}
    category_files = os.listdir(trainfloder)
    for category in category_files:
        # assert category[:-4] in labels
        with open(trainfloder+"/"+category,"r",encoding="utf-8") as file:
            for line in file.readlines():
                sentence=line.strip()
                if category[:-4] in data_dict:
                    data_dict[category[:-4]].append(sentence)
                else:
                    data_dict[category[:-4]]=[sentence]
    return data_dict
...
class OmniglotTask(object):
    ...
    def __init__(self, dict_data, num_classes, train_num, test_num,name="train"):
        self.character_folders = list(dict_data.keys())
        self.num_classes = num_classes
        self.train_num = train_num
        self.test_num = test_num

        self.class_folders = random.sample(self.character_folders, self.num_classes)

        labels = np.array(range(len(self.class_folders)))
        labels = dict(zip(self.class_folders, labels))
        samples = dict()
        self.train_labels =[]
        self.test_labels =[]
...
class Omniglot(FewShotDataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(image), label,self.task.class_folders
